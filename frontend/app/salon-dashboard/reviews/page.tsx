  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-[#111827] text-2xl md:text-3xl font-bold">Reviews</h1>
          <p className="text-[#6B7280] text-sm mt-1">Customer feedback</p>
        </div>
      </div>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-[12px] font-medium border transition-all ${
              ratingFilter === rating
                ...
                : "bg-white text-[#6B7280] border-[#E5E7EB] hover:border-[#EC4899]"
            }`}
          >
      ...
      {loading ? (
        <div className="text-center py-12 text-[#9CA3AF]">Loading reviews...</div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-12 text-[#9CA3AF]">No reviews yet.</div>
      ) : (
        <div className="space-y-4">
          {filtered.map((review) => (
            <div key={review.id} className="bg-white rounded-2xl border border-[#E5E7EB]/60 p-5 shadow-sm">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-[14px] text-[#111827]">{review.user?.full_name || "Anonymous"}</p>
                ...
                <div className="flex items-center gap-0.5">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <Star key={s} size={14} className={s <= review.rating ? "text-[#F5C842] fill-[#F5C842]" : "text-[#E5E7EB]"} />
                  ))}
                </div>
              </div>
              <p className="text-[14px] text-[#111827] leading-relaxed">{review.comment}</p>

              {/* Owner Reply */}
              {review.owner_reply && (
                <div className="mt-4 pl-4 border-l-2 border-[#EC4899] bg-pink-50 rounded-r-xl p-3">
              ...
              {!review.owner_reply && (
                <div className="mt-4 flex gap-2">
                  <input
                    ...
                    className="flex-1 h-12 rounded-xl border border-[#E5E7EB] px-4 text-sm focus:ring-2 focus:ring-[#EC4899]/20 focus:border-[#EC4899]"
                  />
                  <button
